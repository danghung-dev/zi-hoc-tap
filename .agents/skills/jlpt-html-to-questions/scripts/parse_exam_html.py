#!/usr/bin/env python3
"""Extract Part 1 (言語知識・読解) of a gojapan.vn JLPT mock-exam HTML into an
intermediate JSON structure.

This script does the MECHANICAL extraction only: it pulls out every 問題 (mondai),
its instruction, each question's stem (preserving <u> tags, ★ markers and [n]
blank markers), the four answer choices, and any shared passage / image blocks.

It deliberately does NOT decide which answer is correct and does NOT write any
explanation — the source HTML contains no answer key (scoring happens server-side),
so correctness and the Vietnamese explanations are authored later by the model that
reads this output. Keeping extraction separate from judgement avoids hand-copying
Japanese text (full-width spaces, tab padding, <u> tags) which is error-prone.

Usage:
    python parse_exam_html.py path/to/dethi.html [-o out.json]

Output (stdout or -o): see the README at the bottom of this file for the shape.
"""
...
def norm(s):
    """Collapse ASCII whitespace runs to a single space; keep full-width space (　).

    Layout in scramble questions uses 　 (U+3000) between fragments, so we must not
    touch it, but the source pads labels with tabs/newlines which we do collapse.
    """
    if not s:
        return ""
...
def part_of_question_row(row):
    """Return the part id (p1/p2) for a question row, read from a radio's name."""
...
def extract_options(answer_div):
    """Return [{index, text}] from a .answer block's radio + label pairs."""
    options = []
...
def extract_question(row):
    """Parse a single question row (div.p-3.d-flex.border-bottom)."""
    ...
    if content is not None:
        ...
        # Strip <img> from the HTML we keep (srcs captured separately), keep <u>.
...
def extract_passage(div):
    """Parse a shared passage / image block (div.p-3.border-bottom without d-flex)."""
...
def extract_mondai(mondai_div, want_part="p1"):
    ...
    passages, questions = [], []
    # Walk direct children in document order so passages stay attached to the
    ...
    for child in mondai_div.find_all("div", recursive=False):
        if is_question_row(child):
            if part_of_question_row(child) != want_part:
                return None  # belongs to another part (e.g. listening)
    ...
    # Cloze questions mark blanks as [1]..[n] in the shared passage, and each
    # question's content is just that bare blank number.
...
def parse(html, want_part="p1"):
    ...
    for mondai_div in soup.find_all("div", class_="mondai"):
        ...
        if section_title is None:
            # nearest preceding <h2> inside the same part wrapper
...
def main():
    ...
    ap.add_argument(
        "--part",
        default="p1",
        help="Which part to extract (default p1 = language knowledge + reading)",
    )
    args = ap.parse_args()
